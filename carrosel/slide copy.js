...
function initSlider(options) {
        ...
        if (controls) {
                createControlButtons();  // Create control buttons if enabled
                controlsButtons = document.querySelectorAll('[data-slide="control-button"]');
        }

        createSlideClones();  // Clone slides for a continuous loop effect
        setVisibled({ index: startAtIndex + slidesPerView, animate: false });  // Set initial slide

        if (state.autoPlay) {
                setAutoPlay();  // Set autoplay if enabled
        }

        setListeners();  // Add event listeners for user interactions
}
initSlider({
        startAtIndex: 0,
        autoPlay: true,
